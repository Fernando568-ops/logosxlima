...
function addPrim(type){
  ...
  const layerLocked = cadSt.layerState && cadSt.layerState[layer] && cadSt.layerState[layer].locked;
  if(layerLocked){ setMsg('Layer is locked'); return; }
...
function deleteSel(){
  if(!selected){ setMsg('Select an object first, then click Delete'); return; }
  const name = selected.userData.name || 'object';
  removeObj(selected);
  // removeObj already nulls selected if it matches; ensure gizmo is gone
  selected = null;
  if(gizmoGroup){ scene.remove(gizmoGroup); gizmoGroup = null; }
...
function clearScene(){
  // Note: confirm() is blocked in iframes; just clear directly
...
# Safely inject API credentials into the HTML via string replacement
HTML = HTML.replace('"%%APIKEY%%"', json.dumps(OPENROUTER_API_KEY))
HTML = HTML.replace('"%%MODEL%%"', json.dumps(OPENROUTER_MODEL_DEFAULT))
