...
with st.sidebar:
    ...
    # Model selector — overrides secrets.toml at runtime
    MODEL_OPTIONS = {
        "claude-haiku-4-5 (fast & cheap)":       "anthropic/claude-haiku-4-5",
        "claude-3-haiku (fast & cheap)":          "anthropic/claude-3-haiku",
        "claude-3.5-haiku (balanced)":            "anthropic/claude-3.5-haiku",
        "claude-3.5-sonnet (powerful)":           "anthropic/claude-3.5-sonnet",
        "claude-sonnet-4-6 (latest sonnet)":      "anthropic/claude-sonnet-4-6",
        "gpt-4o-mini (cheap OpenAI)":             "openai/gpt-4o-mini",
        "gpt-4o (powerful OpenAI)":               "openai/gpt-4o",
        "gemini-flash-1.5 (free tier)":           "google/gemini-flash-1.5",
        "mistral-7b (very cheap)":                "mistralai/mistral-7b-instruct",
    }
    # Find which label matches the secrets model (if any)
    default_label = next(
        (lbl for lbl, val in MODEL_OPTIONS.items() if val == OPENROUTER_MODEL),
